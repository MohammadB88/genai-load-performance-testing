...
#   2. Create/update the aiperf-hf-token Secret (prompts for the token).
...
# repo. Prompted for here regardless, since it's cheap to skip (leave blank)
# and the Job manifests already mark the secret lookup as optional.
if [ -z "${HF_TOKEN:-}" ]; then
...
if [ -n "$HF_TOKEN" ]; then
    ...
    echo "Secret aiperf-hf-token created/updated in namespace '$NAMESPACE'."
else

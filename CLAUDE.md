...
- **Charting in notebooks: avoid Kaleido/Chrome.** `aiperf_uc1_synthetic_profiling.ipynb` §7 uses Plotly's interactive `fig.show()` rendering (kernel-side HTML/JS, zero external deps) instead of `aiperf plot`'s PNG export, because PNG export goes through Kaleido which requires a headless Chrome/Chromium install — not launchable in locked-down sandboxes (e.g. OpenShift AI workbenches: no sandbox permissions, missing shared libs, no writable browser profile dir). If adding charts elsewhere in `notebooks/`, follow the same interactive-HTML pattern rather than reintroducing a Chrome dependency.
